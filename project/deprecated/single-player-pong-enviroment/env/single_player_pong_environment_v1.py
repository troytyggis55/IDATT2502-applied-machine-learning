...
class SinglePlayerPongEnvironment(ParallelEnv):
    metadata = {'name': "single_player_pong_v0"}
    ...
    def step(self, actions):
        ...
        if self.pong_x < 0:
            if self.left_paddle_y - self.paddle_length < self.pong_y < self.left_paddle_y + self.paddle_length:
                ...
                self.pong_vx = self.pong_speed * np.cos(np.pi * edge_hit / 32)
                self.pong_vy = self.pong_speed * np.sin(np.pi * edge_hit / 32)

                self.pong_speed = min(self.max_pong_speed,
    ...
    def render(self):
        ...
        pygame.draw.rect(self.screen, (
            ...
            255,
            255), pygame.Rect(0, (
                self.left_paddle_y - self.paddle_length) * self.height, 10,
                              self.paddle_length * 2 * self.height))
        pygame.draw.circle(self.screen, (
            255,
            255,
            255), (
                               int(self.pong_x * self.width),
                               int(self.pong_y * self.height)), 5)
    ...
    def observation_space(self, agent):
        return spaces.Box(low=np.array([
            0.0,
            0.0,
            -1.0,
            -1.0,
